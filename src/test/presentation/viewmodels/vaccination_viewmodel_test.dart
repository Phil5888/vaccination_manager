...
void main() {
  ...
  ProviderContainer makeContainer({int? activeUserId}) {
    ...
    return ProviderContainer(overrides: [
      activeUserProvider
          .overrideWith(() => FakeActiveUserNotifier(user)),
  ...
  group('VaccinationViewModel', () {
    test('build() with active user returns that user\'s entries', () async {
      fakeRepo.seedAll([
        ...
        Fixtures.singleShotPast(name: 'MMR', userId: 2),
      ]);
    ...
    test('saveSeries() adds entries to state', () async {
      ...
      await container
          ...
          .saveSeries(Fixtures.threeShots(userId: 1));
    ...
    test('saveSeries() replaces existing series with same name', () async {
      ...
      final twoShots = [
        Fixtures.threeShots(userId: 1).first.copyWith(totalShots: 2),
        Fixtures.threeShots(userId: 1)[1].copyWith(totalShots: 2),
      ];
    ...
    test('deleteSeries() removes all shots in the series', () async {
      fakeRepo.seedAll([
        ...Fixtures.threeShots(name: 'COVID-19', userId: 1),
        Fixtures.singleShotPast(name: 'Flu', userId: 1),
      ]);
      final container = makeContainer(activeUserId: 1);
      addTearDown(container.dispose);
    ...
    test('saveSeries() does not affect another user\'s entries', () async {
      ...
      await container.read(vaccinationProvider.notifier).saveSeries([
        Fixtures.singleShotPast(name: 'Tetanus', userId: 1),
      ]);
