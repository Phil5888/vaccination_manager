...
class FakeCalendarRepository implements CalendarRepository {
  ...
  FakeCalendarRepository({bool supportsNative = true}) : _supportsNative = supportsNative;

  @override
  bool get supportsNativeCalendar => _supportsNative;

  @override
  Future<String?> createEvent({required String title, required DateTime date, required String notes, required int alarmMinutesBefore}) async {
    final returnedId = 'cal-event-${++_eventCounter}';
    createEventCalls.add({'title': title, 'date': date, 'notes': notes, 'alarmMinutesBefore': alarmMinutesBefore, 'returnedId': returnedId});
    return returnedId;
  }
  ...
  @override
  String exportIcs({required String title, required DateTime date, required String notes, required int alarmMinutesBefore}) => ''; // not under test here
}
...
class FakeNotificationRepository implements NotificationRepository {
  ...
  @override
  Future<int> scheduleNotification({required int id, required String title, required String body, required DateTime scheduledDate}) async {
...
class FakeSyncedEventRepository implements SyncedEventRepository {
  ...
  @override
  Future<List<SyncedEventRecord>> getForShot(int vaccinationId) async => List.of(_seeded[vaccinationId] ?? []);

  @override
  Future<List<SyncedEventRecord>> getForUser(int userId) async => _seeded.values.expand((r) => r).toList();
...
VaccinationEntryEntity _shot({int id = 1, int userId = 1, String name = 'Flu', int shotNumber = 1, int totalShots = 1, DateTime? vaccinationDate, DateTime? nextVaccinationDate}) =>
    VaccinationEntryEntity(id: id, userId: userId, name: name, shotNumber: shotNumber, totalShots: totalShots, vaccinationDate: vaccinationDate, nextVaccinationDate: nextVaccinationDate);
...
const _prefsAll = NotificationPreferenceEntity(notificationsEnabled: true, calendarSyncEnabled: true, reminderAdvanceDays: 1);

/// Preferences with every integration point explicitly disabled.
const _prefsNone = NotificationPreferenceEntity(notificationsEnabled: false, calendarSyncEnabled: false, reminderAdvanceDays: 7);
...
void main() {
  ...
  group('ExportIcsUseCase', () {
    ...
    test('empty shots list → valid VCALENDAR wrapper present, no VEVENT emitted', () {
    ...
    test('single shot with a past date → contains one VEVENT with correct '
        'SUMMARY and UID', () {
      // Past shots must still be exportable (users may want historical
      // records in their calendar app).
      final shot = _shot(id: 42, userId: 7, name: 'Flu', shotNumber: 1, totalShots: 1, vaccinationDate: _yesterday);
    ...
    test('multi-shot series with one null-dated shot → one VEVENT per dated '
        'shot, null-dated shot is skipped', () {
      // Shot 2 has no date — it must not appear in the export.
      final shots = [_shot(id: 1, shotNumber: 1, totalShots: 3, vaccinationDate: _yesterday), _shot(id: 2, shotNumber: 2, totalShots: 3, vaccinationDate: null), _shot(id: 3, shotNumber: 3, totalShots: 3, vaccinationDate: _in30Days)];

      final ics = useCase(shots: shots, alarmMinutesBefore: 15);

      final eventCount = 'BEGIN:VEVENT'.allMatches(ics).length;
      expect(eventCount, 2, reason: 'null-dated shot must be omitted from the output');
    });

    // ── VALARM trigger ───────────────────────────────────────────────────

    test('VALARM trigger reflects alarmMinutesBefore parameter', () {
    ...
    test('totalShots > 1 → SUMMARY contains "Shot X of Y" position label', () {
      ...
      final shot = _shot(id: 1, name: 'COVID-19', shotNumber: 2, totalShots: 3, vaccinationDate: _in30Days);
    ...
    test('totalShots == 1 → SUMMARY does NOT contain "Shot" suffix', () {
      // A single-dose vaccine should not emit "Shot 1 of 1" — that label
      // is noise and degrades the calendar event title.
      final shot = _shot(id: 1, name: 'Flu', shotNumber: 1, totalShots: 1, vaccinationDate: _in30Days);

      final ics = useCase(shots: [shot], alarmMinutesBefore: 60);

      final summaryLine = ics.split('\n').firstWhere((l) => l.startsWith('SUMMARY:'));
      expect(summaryLine, isNot(contains('Shot')));
    });

    // ── alarmMinutesBefore = 0 ───────────────────────────────────────────

    test('alarmMinutesBefore=0 → TRIGGER:-PT0M (zero-minute alarm is valid RFC-5545)', () {
      final shot = _shot(id: 1, vaccinationDate: _in30Days);
    ...
    test('shot with null vaccinationDate but set nextVaccinationDate → included '
      ...
      final shot = _shot(id: 1, vaccinationDate: null, nextVaccinationDate: _in30Days);
      final ics = useCase(shots: [shot], alarmMinutesBefore: 30);
      expect(ics, contains('BEGIN:VEVENT'));
      expect(ics, contains('END:VEVENT'));
    });

    test('shot with both vaccinationDate and nextVaccinationDate null → skipped', () {
      final shot = _shot(id: 1, vaccinationDate: null, nextVaccinationDate: null);
      final ics = useCase(shots: [shot], alarmMinutesBefore: 30);
      expect(ics, isNot(contains('BEGIN:VEVENT')));
    });
    ...
    test('two shots in a series produce distinct UIDs', () {
      final shots = [_shot(id: 1, shotNumber: 1, totalShots: 2, vaccinationDate: _yesterday), _shot(id: 2, shotNumber: 2, totalShots: 2, vaccinationDate: _in30Days)];
      final ics = useCase(shots: shots, alarmMinutesBefore: 30);

      final uids = RegExp(r'UID:([^\r\n]+)').allMatches(ics).map((m) => m.group(1)).toSet();
  ...
  group('SyncCalendarUseCase', () {
    ...
    SyncCalendarUseCase makeUseCase({FakeCalendarRepository? calendarRepo, FakeNotificationRepository? notificationRepo, FakeSyncedEventRepository? syncedEventRepo}) =>
        SyncCalendarUseCase(calendarRepo: calendarRepo ?? FakeCalendarRepository(), notificationRepo: notificationRepo ?? FakeNotificationRepository(), syncedEventRepo: syncedEventRepo ?? FakeSyncedEventRepository());

    // ── Both integrations disabled ───────────────────────────────────────

    test('calendarSyncEnabled=false AND notificationsEnabled=false → '
        'no createEvent / scheduleNotification calls, nothing inserted', () async {
      ...
      await makeUseCase(calendarRepo: cal, notificationRepo: notif, syncedEventRepo: synced).call(
    ...
    test('calendarSyncEnabled=true → createEvent called for future-dated shots; '
        'past and null-dated shots are skipped; synced records inserted', () async {
      ...
      const prefs = NotificationPreferenceEntity(notificationsEnabled: false, calendarSyncEnabled: true, reminderAdvanceDays: 1);
    ...
    test('notificationsEnabled=true → scheduleNotification called for each '
        'future shot whose notify date is also in the future', () async {
      ...
      const prefs = NotificationPreferenceEntity(notificationsEnabled: true, calendarSyncEnabled: false, reminderAdvanceDays: 1);

      await makeUseCase(calendarRepo: cal, notificationRepo: notif, syncedEventRepo: synced).call(
        shots: [
          _shot(id: 10, shotNumber: 1, vaccinationDate: _in30Days),
          _shot(id: 11, shotNumber: 1, vaccinationDate: _in30Days.add(const Duration(days: 10))),
    ...
    test('idempotent: stale calendar event and notification are deleted before '
        'fresh ones are created', () async {
      ...
      synced.seed(1, [SyncedEventRecord(userId: 1, vaccinationId: 1, calendarEventId: 'stale-cal-event', notificationId: 9001, syncedAt: DateTime.now().subtract(const Duration(days: 7)))]);
    ...
    test('supportsNativeCalendar=false → createEvent not called even when '
        'calendarSyncEnabled=true', () async {
      ...
      expect(cal.createEventCalls, isEmpty, reason: 'createEvent must not be called when supportsNativeCalendar=false');
    });
  });
  ...
  group('CancelNotificationsUseCase', () {
    /// Convenience factory.
    CancelNotificationsUseCase makeUseCase({FakeCalendarRepository? calendarRepo, FakeNotificationRepository? notificationRepo, FakeSyncedEventRepository? syncedEventRepo}) =>
        CancelNotificationsUseCase(calendarRepo: calendarRepo ?? FakeCalendarRepository(), notificationRepo: notificationRepo ?? FakeNotificationRepository(), syncedEventRepo: syncedEventRepo ?? FakeSyncedEventRepository());

    // ── Empty list ───────────────────────────────────────────────────────

    test('empty list → no repo calls made at all', () async {
      ...
      await makeUseCase(calendarRepo: cal, notificationRepo: notif, syncedEventRepo: synced).callForShots([]);
    ...
    test('shot IDs that have synced records → deleteEvent and cancel called per '
        'record; deleteForShot called for each ID', () async {
      ...
      synced.seed(10, [SyncedEventRecord(userId: 1, vaccinationId: 10, calendarEventId: 'evt-10', notificationId: 1010, syncedAt: DateTime.now())]);
      synced.seed(20, [SyncedEventRecord(userId: 1, vaccinationId: 20, calendarEventId: 'evt-20', notificationId: 2020, syncedAt: DateTime.now())]);

      await makeUseCase(calendarRepo: cal, notificationRepo: notif, syncedEventRepo: synced).callForShots([10, 20]);
    ...
    test('shot IDs with no synced records → no deleteEvent / cancel calls, '
      ...
      await expectLater(makeUseCase(calendarRepo: cal, notificationRepo: notif, syncedEventRepo: synced).callForShots([99, 100]), completes);
